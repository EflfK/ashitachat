...
local function read_string(buffer, max_length)
    local value = ffi.string(buffer, max_length);
    return value:gsub('\x00.*$', '');
end

ashita.events.register('plugin_event', 'spectral_focus_modal_event_cb', function (e)
    if (e == nil or e.name ~= 'ashitamove_modal_v1' or e.data_raw == nil) then
        return;
    end

    local required = ffi.sizeof('ashitamove_modal_event_t');
    ...
    M.menu_name = read_string(payload.menu_name, 31);
end);
